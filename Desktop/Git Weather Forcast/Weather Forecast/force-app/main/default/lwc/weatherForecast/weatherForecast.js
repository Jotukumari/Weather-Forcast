...
import jk from '@salesforce/apex/wheatherController.A';
import pk from '@salesforce/apex/wheatherController.D';
import retrieveContactData from '@salesforce/apex/wheatherController.B';
import retrieveContactData2 from '@salesforce/apex/wheatherController.C';
...
export default class WeatherForecast extends LightningElement {
   ...
   /* connectedCallback() {
        jk({keyd:this.dt})
            ...
            .catch(error => {
                System.debug('data error will be:::::' + error);
            })
    }*/
    //here search data
    
    @track accountList=[];
    @wire(retrieveContactData,{keySearch:'$accountName'})
    retervivedata({error,data}){
        if(data){
